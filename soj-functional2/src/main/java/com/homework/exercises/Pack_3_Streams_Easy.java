...
public class Pack_3_Streams_Easy {
    ...
    public void exercise_1_map_collect() {
        ...
        result = numbers.stream().map(i -> i/2).collect(toSet());
    ...
    public void exercise_2_map() {
        ...
        //TODO write your code here
        result = strings.stream().map(s -> Integer.parseInt(s) % 13).distinct().sorted().collect(toList());
    ...
    public void exercise_3_reduce() {
        ...
        //TODO write your code here
        result = strings.stream().reduce((s1, s2) -> s1.length() > s2.length() ? s1 : s2).get().length();
...
//        sum = list.stream().sum(); // doesn't compile, not an int stream
    ...
    public void exercise_4_filter_limit() {
        ...
        result = numbers.filter(i -> i % 17 == 0).limit(5).mapToObj(Integer::valueOf).collect(toList());
    ...
    public void example_7_flatMap_collectJoining() {
        ...
        Stream<Integer> flattenedStream = streamOfStream.flatMap(Function.identity());
